...
public class General<T>
{
    ...
    public void ShowInfoAdmin()
    {
        for (int i = 0; i < currentIndex; i++)
        {
            if (intArray[i].item is Admin admin) // Проверяем, является ли item объектом Admin
            {
                admin.ShowInfo(); // Вызываем метод ShowInfo объекта Admin
            }
            else
            {
                Console.WriteLine(intArray[i].item); // Выводим item по умолчанию
            }
        }
    }
    ...
    public void ShowInfoPrem()
    {
        for (int i = 0; i < currentIndex; i++)
        {
            if (intArray[i].item is Premium premium) // Проверяем, является ли item объектом Admin
            {
                premium.ShowInfo(); // Вызываем метод ShowInfo объекта Admin
            }
            else
            {
                Console.WriteLine(intArray[i].item); // Выводим item по умолчанию
...
class Application
{
    ...
    public class CustomLengthException : Exception
    {
        public CustomLengthException(string message) : base(message) { }
    }
    ...
    class Program
    {
        static void Main(string[] args)
        {
            try
            {
                Console.WriteLine("Введите строку (не более 5 символов): ");
                string input = Console.ReadLine();

                if (input.Length > 5)
                {
                    throw new CustomLengthException("Длина строки не должна превышать 5 символов.");
                }

                if (string.IsNullOrWhiteSpace(input))
                {
                    throw new CustomArgumentException("Строка не должна быть пустой или содержать только пробелы.");
                }
            ...
            {
                Console.WriteLine(ex.Message + "" + "Ошибка аргумента.");
            }
            catch (CustomLengthException ex)
            {
                Console.WriteLine(ex.Message + "" + "Ошибка длины.");
            }
            ...
            catch(InvalidOperationException ex)//std::invalid_argument
            {
                Console.WriteLine(ex.Message + " " + "Объект не инициализирован.");
            }

            Console.WriteLine("//...........................................7lab...............................................................//");
            try
            {
                Console.Write("Bведите число: "); //2147483648
                int num = int.Parse(Console.ReadLine());
            }
            catch (FormatException ex)
            {
                Console.WriteLine(ex.Message + " " + "Перхват исключения типа int по формату.");
            }
            catch (OverflowException ex)//std::out_of_range или std::length_error
            {
                Console.WriteLine(ex.Message + " " + "Перехват исключения типа int по переполнению.");
            }
            finally
            {
                Console.WriteLine("Блок кода, который выполняется всегда.(int)");
            }
            ...
            {
                Console.WriteLine(ex.Message + " " + "Перехват исключения типа string(с помощью генерации).");
            }
            finally
            {
                Console.WriteLine("Блок кода, который выполняется всегда.(string)");
            }
            ...
            try
            {
                int i = 1;
                int j = 0;
                Console.WriteLine("Деление i на j" + i/j);
            }
            catch (Exception ex)
            {
                Console.WriteLine(ex.Message + " " + "Перехват любой исключительной ситуации.");
            }
